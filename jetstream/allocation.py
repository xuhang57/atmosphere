...
def get_user_allocations(username, tacc_api=None, resource_name='Jetstream', raise_exception=False):
    if not tacc_api:
        tacc_api = settings.TACC_API_URL
...
def get_or_create_allocation_source(api_allocation, update_source=False):
    try:
        title = "%: %s" % (api_allocation['project'], api_allocation['justification'])
        source_id = alloc['id']
        compute_allowed = int(alloc['computeAllocated'])
    except:
        raise TASAPIException("Malformed API Allocation - %s" % api_allocation)
